...
class ImageProcessing {
	...
	public static Mat optFingerP(Mat mat, int mode, IDebugImage di) {
		ImageProcessing processActivity = new ImageProcessing();
		return processActivity.getProcessedImage(mat, mode, di);
	}
     ...
     */
    Mat getProcessedImage(Mat imageColor, int mode, IDebugImage di) {
        this.progressStep = 0;

        if( di != null )	di.writeMat2( "entrada",imageColor);
        //Mat imageColor = bytesToMat(data);

        //imageColor = rotateImage(imageColor);
        //if( di != null )	di.writeMat2( "rotateImage",imageColor);
        //imageColor = cropFingerprint(imageColor);
        imageColor = skinDetection(imageColor,  mode,  di);
        if( di != null )	di.writeMat2( "skinDetection",imageColor);
        ...
        if( di != null )	di.writeMat2( "cvtColor",image);
        ...
        if( di != null )	di.writeMat2( "equalized",image);
        ...
        if( di != null )	di.writeMat2( "floated",floated);

        Mat skeleton = getSkeletonImage(floated, rows, cols);
        incrementProgress();
        if( di != null )	di.writeMat2( "skeleton",skeleton);

        skeleton = thinning(skeleton);
        incrementProgress();
        if( di != null )	di.writeMat2( "thinning",skeleton);

        Mat skeleton_with_keypoints = detectMinutiae(skeleton, 1);
        incrementProgress();
        if( di != null )	di.writeMat2( "skeleton_with_keypoints",skeleton_with_keypoints);
    ...
    public Mat skinDetection(Mat src, int mode, IDebugImage di) {
        ...
        if( di != null )	di.writeMat2( "hsvFrame",hsvFrame);
        ...
        if( di != null )	di.writeMat2( "inRange",skinMask);
...
//        skinMask.copyTo(currentSkinMask);
        ...
        if( di != null )	di.writeMat2( "skinMask",skinMask);
        ...
        if( di != null )	di.writeMat2( "GaussianBlur",src);
    ...
    private Mat mat2Bitmap(Mat src, int code) {
