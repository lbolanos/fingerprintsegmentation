...
public class ProcessActivity {
	...
	public static Mat optRealFingerP(Mat mat, int mode, IDebugImage di) {
		...
		return processActivity.processImage(mat, mode, di);
	}

    /**
     * Process the image to get the skeleton.
     */
    private Mat processImage(Mat MatSnapShot, int mode, IDebugImage di ) {
    	//MatSnapShot.convertTo(MatSnapShot, CvType.CV_8UC1);
		Imgproc.cvtColor(MatSnapShot, MatSnapShot, Imgproc.COLOR_RGB2GRAY);
        ...
        if( di != null )	di.writeMat2( "matRidgeSegment",matRidgeSegment);
        ...
        if( di != null )	di.writeMat2( "matRidgeOrientation",matRidgeOrientation);
        ...
        if( di != null )	di.writeMat2( "matFrequency",matFrequency);
        ...
        if( di != null )	di.writeMat2( "matRidgeFilter",matRidgeFilter);

        // step 5: enhance image after ridge filter
        Mat matEnhanced = new Mat(imgRows, imgCols, CvType.CV_8UC1);
        enhancement(matRidgeFilter, matEnhanced, blockSize, di );
        
        if( di != null )	di.writeMat2( "matEnhanced",matEnhanced);

        // set process result
        Mat matResult = matEnhanced.clone();
     ...
     * @param di 
     */
    private void enhancement(Mat source, Mat result, int blockSize, IDebugImage di) {

        //Mat paddedMask = imagePadding(MatSnapShotMask, blockSize);

        // apply the original mask to get rid of extras
        Core.multiply(source, source, result, 1.0, CvType.CV_8UC1);

        // apply binary threshold
        Imgproc.threshold(result, result, 0, 255, Imgproc.THRESH_BINARY);

        if( di != null )	di.writeMat2( "result",result);
